...
export abstract class BaseSkillTreeRenderer implements ISkillTreeRenderer {
    ...
    private DrawBackground = (): void => {
        ...
        var asset: ISpriteSheetAsset = {
            ...
            key: this.skillTreeData.tree.slice(0, 5) === "Atlas" ? "atlasBackground" : "background",
            icon: this.skillTreeData.tree.slice(0, 5) === "Atlas" ? "AtlasPassiveBackground" : background
        }
    ...
    private DrawNodes = (layer: RenderLayer, nodes: { [id: string]: SkillNode }, all: { [id: string]: SkillNode }, options: DrawNodeOptions): void => {
        ...
        for (const id in nodes) {
            ...
            if (!node.isAscendancyStart && icon !== "") {
                if (this.skillTreeData.tree.slice(0, 5) === "Atlas" && node.isMastery && node.is(SkillNodeStates.Hovered)) {
    ...
    RenderHighlight = (): void => {
        ...
        for (const id in desiredNodes) {
            if(startNodes.indexOf(id) === -1 && !desiredNodes[id].isAscendancyStart)
